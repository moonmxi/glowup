...
class ShowcaseState extends ChangeNotifier {
  ...
  Future<void> loadLessonPlans({
    String? gradeLevel,
    String? subject,
    String sortBy = 'popular', // popular, recent, rating
  }) async {
    _setLoading(true);
    try {
      // TODO: 调用后端API获取教案列表
      // 目前使用现有的内容feed系统，kind为'lesson-plan'
      await loadFeed(
        scope: _scope,
      ...
      if (gradeLevel != null) {
        _items = _items.where((item) {
          ...
          return meta['gradeLevel'] == gradeLevel;
        }).toList();
      }

      if (sortBy == 'rating') {
        _items.sort((a, b) {
          final aRating = (a.metadata['rating'] as num?)?.toDouble() ?? 0.0;
          final bRating = (b.metadata['rating'] as num?)?.toDouble() ?? 0.0;
  ...
  /// 为教案评分
  Future<bool> rateLessonPlan(String itemId, int rating) async {
    if (_auth.token == null) {
      _error = '请先登录后再评分';
      notifyListeners();
      return false;
    ...
    try {
      // TODO: 调用后端API提交评分
      // await AppApiService.rateLessonPlan(
      //   token: _auth.token!,
      //   itemId: itemId,
      //   rating: rating,
      // );
      ...
      if (index != -1) {
        ...
        final currentRating = (item.metadata['rating'] as num?)?.toDouble() ?? 0.0;
        final currentCount = (item.metadata['ratingCount'] as num?)?.toInt() ?? 0;
        ...
        updatedMetadata['rating'] = newRating;
        updatedMetadata['ratingCount'] = newCount;

        // 更新item (需要重新构造，因为ShowcaseItem是immutable的)
        // 这里简化处理，实际应该从服务器重新获取
        notifyListeners();
      }
  ...
  /// 添加教学经验评论
  Future<bool> addTeachingExperience({
    ...
    try {
      // TODO: 调用后端API添加评论
      // await AppApiService.addLessonPlanComment(
      //   token: _auth.token!,
      //   itemId: itemId,
      //   content: content,
      //   rating: rating,
      ...
      // 评论成功后重新加载该项目
      await loadItems(category: _activeCategory);
  ...
  /// 收藏教案
  Future<bool> bookmarkLessonPlan(String itemId) async {
    if (_auth.token == null) {
      _error = '请先登录后再收藏';
      notifyListeners();
      return false;
    ...
    try {
      // TODO: 调用后端API收藏
      // await AppApiService.bookmarkLessonPlan(
      //   token: _auth.token!,
      //   itemId: itemId,
      // );
      ...
      if (index != -1) {
        final item = _items[index];
        final bookmarks = (item.metadata['bookmarks'] as num?)?.toInt() ?? 0;
        
        final updatedMetadata = Map<String, dynamic>.from(item.metadata);
        updatedMetadata['bookmarks'] = bookmarks + 1;
        updatedMetadata['isBookmarked'] = true;
